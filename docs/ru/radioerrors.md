...
![Заблокированный пульт](../assets/lockradio.png)
...
1. Соединение с приемником отсутствует:
    ...
    ![Нет соединения с приемником](../assets/connectionLost.png)

2. Соединение с приемником установлено:

    На RadioMaster Pocket понять, что дрон подключен, можно по стабильному уровню сигнала (телеметрии) в верхнем углу экрана (значок антенны и dBm) и отсутствию предупреждения «Telemetry Lost». 

Если соединение отсутствует, то:

1. Проверьте, что приемник включен (моргает зелёный светодиод). Если светодиод горит непрерывно зелёным, то значит связь установлена с другим пультом.
